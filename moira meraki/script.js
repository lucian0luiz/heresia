...
const STORAGE_KEY = "moira-meraki-sheet-v8";
...
function renderTextList(id, listName, withIcon = true) {
  ...
  el.innerHTML = data[listName].map((text, i) => {
    ...
    return `
      <li class="${listName === "talents" && i === 3 ? "unlocked-talent" : ""}">
        <span class="cell-edit" data-list="${listName}" data-index="${i}">${formatted}</span>
...
function exportJson() {
  ...
  a.href = url;
  a.download = "moira-meraki-ficha.json";
  a.click();
